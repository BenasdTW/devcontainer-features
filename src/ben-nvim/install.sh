...
apt-get update && apt-get install -y ripgrep fd-find python3-venv luarocks npm curl tar
apt-get install -y shfmt
# npm i @johnnymorganz/stylua-bin

# Install stylua
curl -fLo /usr/local/bin/stylua https://github.com/JohnnyMorganz/StyLua/releases/latest/download/stylua-linux.zip && chmod +x /usr/local/bin/stylua
